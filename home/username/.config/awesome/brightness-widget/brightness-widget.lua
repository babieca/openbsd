...
function Brightness:new(args)
	...
	obj.widget:set_image(config.."/brightness-widget/icons/4.png")
...
function Brightness:tooltipText()
	return string.sub(self:getBrightness(), 0, 3).."% Brightness"
end

function Brightness:update(status)
	local b = self:getBrightness()
	local img = math.floor((b/100)*7)
	self.widget:set_image(config.."/brightness-widget/icons/"..img..".png")
end
